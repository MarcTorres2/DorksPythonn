...
def getAllFiles(domini:str)->None:
    pass
    query = "site:" + domini + " filetype:txt OR filetype:docx OR filetype:pdf OR filetype:xlsx"
    urls = doGoogleSearch(query)
    #TODO: Implement dork search for getting files from an specific domain (parse URLs required), storing results in binary file'''
    print('Realitzan Cerca a Google...')
    for url in urls:
        r = requests.get(url)
...
def getAllEmails(domini:str)->None:
    ...
    query = "site:" + domini + " (gmail.com OR hotmail.com OR educem.com)"

    input = doGoogleSearch(query,1,True)

    if type(input) == list:
        input = [str(i) for i in input]
        input = ' '.join(input)

    mails = re.findall(r'[\w.-]+@[\w.-]+', input)
...
def getAllSubdomains(domini:str)->None:
    ...
    query = "site:*." + domini +" -site:www." + domini
    input = doGoogleSearch(query,1,True)
    'text = (" ".join(text))'
    ...
    urls = re.findall(r'https?://(?:[-\w.]|(?:%[\da-fA-F]{2}))+', input)

    with open('subdominis.txt', 'w') as file:
        for url in urls:
            file.write(url + "\n")
''' 
